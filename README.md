...
Обработка изображения проходит в несколько этапов:
...
В репозитории используется облегчённая база `music.db` примерно на 100 000 треков. Она предназначена для быстрого локального запуска.
...
Система предназначена для локального запуска и требует видеокарту NVIDIA с достаточным объёмом VRAM для работы мультимодальной модели.
